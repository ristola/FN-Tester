...
// format for every ShackMate ESP32 device (the CYD-4.3-FN-Tester panel,
// this Atom S3 Lite FN pod, rotors, power controllers, SWR meters,
// weather stations, display/button panels, etc).
//
// This file is a copy of ../CYD-4.3-FN-Tester/src/espnow_protocol.h (see
// that repo's ESPNOW_PROTOCOL.md for the full protocol writeup) - there's
// no shared library between the two PlatformIO projects, so any change
// here (especially to SM_DeviceType, which must stay append-only across
// every ShackMate device in the field) needs to be mirrored there by hand.
//
