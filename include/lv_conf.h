...
#define LV_USE_THEME_DEFAULT 1
    ...
    // Kept at 0 deliberately (not the LVGL default of 80): a nonzero value
    // animates style changes (e.g. a button's pressed state) via lv_anim,
    // spreading the redraw across several animation-timer ticks instead of
    // one atomic style change + redraw. On this board that reliably hangs
    // LVGL's software draw dispatcher (draw_task_head never clears, so
    // draw_buf_flush()'s wait loop spins forever) on the second+ redraw of a
    // session - isolated via bisection against LV_THEME_DEFAULT_GROW and
    // widget scrollability, both of which turned out to be unrelated.
